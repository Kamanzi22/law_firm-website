...
function Timeline({ data }: { data: { label: string; count: number }[] }) {
  ...
  return (
    <div className="flex h-32 items-end gap-1 overflow-x-auto">
      {data.map((d, i) => (
        <div key={`${d.label}-${i}`} className="flex h-full min-w-[1.5rem] flex-1 flex-col items-center justify-end gap-1">
          <div
            className="w-full min-h-[2px] rounded-t-sm bg-brand-gold"
      ...
      {stats && (
        <>
          <div className="mt-5 grid grid-cols-2 gap-4">
            <div className="rounded-sm bg-brand-gray-50 p-4">
              <div className="flex items-center gap-2 text-brand-gray-500">
                <Users className="h-4 w-4" aria-hidden="true" />
                <span className="text-xs font-medium uppercase tracking-wide">Visits</span>
              </div>
              <p className="mt-1 font-display text-2xl font-semibold text-brand-navy">{stats.totalVisits}</p>
            </div>
            <div className="rounded-sm bg-brand-gray-50 p-4">
              <div className="flex items-center gap-2 text-brand-gray-500">
